...
### Sources
* *CDC PLACES: Local Data for Better Health: County Data* - [Link](https://chronicdata.cdc.gov/500-Cities-Places/PLACES-County-Data-GIS-Friendly-Format-2022-releas/i46a-9kgh)
>>>>>>> c6483932c0684099e5a6c2e412d7ee5e12efda83
...
### Data Cleaning
The CDC PLACES and EPA NPL datasets were cleaned in a similar fashion. Both datasets were in CSV format and imported into R. The variables names were all cleaned, then the variables of interest were selected and renamed if they were not intuitive to the user. Since the CDC dataset was nationwide, we computed the national average prevalence for cancer and asthma,to compare to the prevalence in Pennsylvania state. Then, we filtered the dataset to only include values for Pennsylvania state. For the EPA NPL dataset, the dataset was already isolated to only include sites in Pennsylvania. 

For the exploratory analyses, the two datasets were then joined by `county` and tidy datasets were used to analyze the outcomes between counties with superfunds versus counties without superfunds. 
...
<br> The exploratory analysis for the  EPA NPL dataset was used to give context to the number of Superfund sites in PA, the mean site score, and where they were located geographically. 
...
=======
We hypothesized this was a function of population size in urban cities. We also believed that the cancer counts may have had to do with potentially more toxic superfund sites. Finally, we were interested in the potential for SES (i.e access to insurance), behavioral factors like smoking, and health attributes like asthma could potentially confound our study.Therefore, we opted to explore whether county population size was impacting this relationship.
>>>>>>> c6483932c0684099e5a6c2e412d7ee5e12efda83
